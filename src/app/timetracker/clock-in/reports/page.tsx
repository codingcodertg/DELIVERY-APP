...
export default async function ReportsPage({ searchParams }: { searchParams: Promise<{ week?: string }> }) {
  ...
  // Apuntaba a /api/clock-in/..., que no existe ni existió: la ruta vive DENTRO del
  // módulo (/timetracker/clock-in/api/...). Roto desde que clock-in se fusionó — el
  // prefijo se insertó en el sitio equivocado para las rutas /api. Lo destapó moverlo.
  const exportHref = (type: string) => `/timetracker/clock-in/api/reports/export?type=${type}&week=${monday}`;
    ...
    <main className="flex-1 w-full max-w-2xl mx-auto p-5 flex flex-col gap-5">
      ...
      <section className="rounded-2xl border border-zinc-200 dark:border-zinc-800 p-5 flex flex-col gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <a href={`/timetracker/clock-in/api/reports/xlsx?week=${monday}`} className={btn("primary", "sm")}>
