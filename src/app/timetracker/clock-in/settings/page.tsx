...
export default async function SettingsPage() {
  ...
  const cards: { href: string; icon: string; label: string; desc: string }[] = [
    ...
    { href: "/timetracker/clock-in/team", icon: "🚚", label: m.vehicles, desc: m.settingsVehiclesDesc },
    // Job sites (geofences) are owner-only.
    ...(isOwner ? [{ href: "/timetracker/clock-in/sites", icon: "📍", label: m.jobSites, desc: m.settingsSitesDesc }] : []),
    { href: "/timetracker/clock-in/account", icon: "⚙️", label: tr.account, desc: m.settingsAccountDesc },
  ];
