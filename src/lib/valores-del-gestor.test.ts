...
import { etiquetaDelGestor, textoQueAbreLaOrden, valorDelGestor, type DeOrdenes } from "./valores-del-gestor";
...
describe("el Gestor de Rutas usa el menú en «Sin asignar» y abre la orden desde el ID y la factura (D-360)", () => {
  ...
  it("la factura abre la orden con el mismo gesto que la tabla de paradas, parando el clic (y el ID, hasta D-408)", () => {
    ...
    const tramo = sinAsignar();
    expect(tramo).not.toContain('<td className="ordno" {...abreLaOrden(d)}>');
...
describe("D-408: lo que se lee en el enlace que abre la orden", () => {
  it("la factura, tal cual; sin factura (o solo espacios), el código de la orden, marcado como no-factura", () => {
    ...
    expect(textoQueAbreLaOrden(mk({ order_no: 501, invoice_num: null }))).toEqual({ texto: "#501", esFactura: false });
    expect(textoQueAbreLaOrden(mk({ order_no: 501, invoice_num: "   " }))).toEqual({ texto: "#501", esFactura: false });
    expect(textoQueAbreLaOrden(mk({ order_no: 7, order_code: "MC-7", order_suffix: "-R", invoice_num: "" }))).toEqual({ texto: "#MC-7-R", esFactura: false });
  });
});
