...
const SIETE = [
  { name: "RDZ Brownsville", lat: 25.9, lng: -97.49 },
  { name: "RDZ Weslaco", lat: 26.16, lng: -97.99 },
  { name: "RDZ Pharr", lat: 26.19, lng: -98.18 },
  { name: "RDZ McAllen", lat: 26.2, lng: -98.23 },
  { name: "RDZ Mission", lat: 26.21, lng: -98.32 },
  { name: "RDZ Edinburg", lat: 26.3, lng: -98.16 },
...
describe("cuál de las siete es la del pedido", () => {
  it("la tienda del pedido es `origen` y las otras seis `otra`", () => {
    const p = papeles("RDZ McAllen");
    expect(p["RDZ McAllen"]).toBe("origen");
    expect(Object.values(p).filter((x) => x === "origen")).toHaveLength(1);
    expect(Object.values(p).filter((x) => x === "otra")).toHaveLength(6);
  });
  it("los cuatro nombres que hoy existen en pedidos reales casan", () => {
    // Medidos en producción por el orquestador: Brownsville (108 pedidos), McAllen (5),
    // Edinburg (2), Pharr (1). Si un renombrado rompiera uno, esta prueba lo dice.
    for (const n of ["RDZ Brownsville", "RDZ McAllen", "RDZ Edinburg", "RDZ Pharr"]) {
      expect(papeles(n)[n], n).toBe("origen");
    }
  });
  it("espacios de sobra y mayúsculas no rompen el destacado", () => {
    ...
    expect(papeles("  rdz mcallen  ")["RDZ McAllen"]).toBe("origen");
    expect(papeles("RDZ  McAllen")["RDZ McAllen"]).toBe("origen");
    expect(nombreNormalizado("  RDZ   Pharr ")).toBe("rdz pharr");
  });
  it("normaliza, pero NO empareja cosas distintas", () => {
    ...
    expect(papeles("RDZ")["RDZ Pharr"]).toBe("otra");
    expect(papeles("McAllen")["RDZ McAllen"]).toBe("otra");
    expect(papeles("RDZ Mcallen ")["RDZ Mission"]).toBe("otra");
  });
  ...
  it("no pierde ni inventa tiendas, y conserva sus coordenadas", () => {
    const salida = tiendasParaElMapa(SIETE, "RDZ Pharr");
    expect(salida).toHaveLength(7);
    expect(salida.map((t) => t.name)).toEqual(SIETE.map((t) => t.name));
    expect(salida[2]).toMatchObject({ name: "RDZ Pharr", lat: 26.19, lng: -98.18, papel: "origen" });
  });
  it("lista vacía: no revienta", () => {
    expect(tiendasParaElMapa([], "RDZ Pharr")).toEqual([]);
  });
});
...
describe("una tienda no se confunde con la entrega, ni la del pedido con las demás", () => {
  ...
  it("la forma distingue sin color: un cuadrado, no una gota ni un círculo", () => {
    ...
    const d = dibujoTienda(estiloTienda("otra"), "RDZ Pharr");
...
describe("el dibujo: uno solo para los dos motores", () => {
  it("el ancla es el centro del cuadrado, no el del lienzo con la etiqueta", () => {
    ...
    const d = dibujoTienda(e, "RDZ Brownsville");
  ...
  it("sin etiqueta permanente no hay texto en el SVG y el lienzo es del tamaño del cuadrado", () => {
    ...
    const d = dibujoTienda(e, "RDZ Weslaco");
  ...
  it("la etiqueta lleva el nombre y el lienzo crece con él", () => {
    const e = estiloTienda("origen");
    const corto = dibujoTienda(e, "RDZ Pharr");
    const largo = dibujoTienda(e, "RDZ Brownsville Superstore");
    expect(corto.svg).toContain(">RDZ Pharr</text>");
  ...
  it("un `&` o un `<` en el nombre no rompen el SVG", () => {
    ...
    const d = dibujoTienda(estiloTienda("origen"), 'Tile & Stone <"RDZ">');
    expect(d.svg).not.toMatch(/[<>&]/g.source ? /&(?!#)/ : /&/);
    expect(d.svg).toContain(">Tile  Stone RDZ</text>");
  });
  it("la URL `data:` de Google lleva exactamente el mismo SVG que Leaflet inserta tal cual", () => {
    ...
    const e = estiloTienda("origen");
    const url = dibujoTiendaUrl(e, "RDZ McAllen");
    expect(decodeURIComponent(url.url.replace("data:image/svg+xml;charset=UTF-8,", ""))).toBe(dibujoTienda(e, "RDZ McAllen").svg);
    expect(url.anclaX).toBe(dibujoTienda(e, "RDZ McAllen").anclaX);
  });
});
...
describe("el cableado: quién pasa qué, y a quién NO", () => {
  ...
  it("el punto rojo de siempre sigue siendo el mismo punto rojo", () => {
    // Los cuatro mapas de despacho no pasan `papel`, así que caen por la rama clásica. Los
    // valores se fijan aquí para que nadie los cambie creyendo que solo toca la ficha.
    expect(TIENDA_CLASICA).toEqual({ fill: "#e11414", diametro: 24, borde: "#fff", grosor: 3 });
    expect(leaflet).toContain("width:${TIENDA_CLASICA.diametro}px");
    expect(google).toContain('r="9"');
  });
