...
/** La factura y el selector de columnas del Gestor de Rutas (D-NEXT): el catálogo, la página y la 137. */
...
describe("se guarda por persona, en su propia clave", () => {
  it("lee y guarda `routes_columns`, no `order_columns`", async () => {
    ...
    expect(await leeColumnas(cliente, "yo", CLAVE_DE_COLUMNAS_DEL_GESTOR)).toEqual({ leida: true, hayFila: true, columnas: { logistics: ["invoice"] } });
