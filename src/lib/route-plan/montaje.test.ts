...
describe("los componentes del plan de ruta se montan bajo sus proveedores", () => {
  it("`PlanDelDia` solo en /routes, y `RutaDelPlan` solo dentro de `PlanDelDia`", () => {
    ...
    expect(dondeSeMonta("PrecisionDelPlan")).toEqual(["src/components/PlanDelDia.tsx"]);
    expect(dondeSeMonta("ComparaConLaHoja")).toEqual(["src/components/PlanDelDia.tsx"]);
  ...
  it("los hooks con proveedor que usan: los tres, y ninguno más que no esté cubierto abajo", () => {
    ...
    expect(hooksDe("src/components/PrecisionDelPlan.tsx")).toEqual(["usePrefs"]);
    expect(hooksDe("src/components/ComparaConLaHoja.tsx")).toEqual(["usePrefs"]);
