...
describe("la base", () => {
  it("lee SOLO la fila propia de `order_columns`, y la sanea", async () => {
    ...
    expect(await leeColumnas(cliente, "yo")).toEqual({ leida: true, hayFila: true, columnas: { logistics: ["stage"] }, orden: {}, anchos: {}, plantillas: [] });
  ...
  it("sin fila: leída y sin fila. Con error —la tabla aún no existe— o sin red: NO leída, y no revienta", async () => {
    expect(await leeColumnas(clienteFalso({ fila: null }).cliente, "yo")).toEqual({ leida: true, hayFila: false, columnas: {}, orden: {}, anchos: {}, plantillas: [] });
    expect(await leeColumnas(clienteFalso({ errorAlLeer: true }).cliente, "yo")).toEqual({ leida: false, hayFila: false, columnas: {}, orden: {}, anchos: {}, plantillas: [] });
    expect(await leeColumnas(clienteFalso({ lanza: true }).cliente, "yo")).toEqual({ leida: false, hayFila: false, columnas: {}, orden: {}, anchos: {}, plantillas: [] });
  });
...
describe("la página de Órdenes", () => {
  ...
  it("ventas sigue con la lista del admin y sin tocar `user_prefs`; los demás pintan YA lo del navegador y luego manda la base", () => {
    // Ventas: sus columnas salen de Ajustes (sin las que le repiten la factura, D-338) y nada de la base las cambia.
