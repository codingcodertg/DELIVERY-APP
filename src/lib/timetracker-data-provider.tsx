...
export function DataProvider({ children, me }: { children: React.ReactNode; me: Employee }) {
  ...
  // update exige `payroll_id IS NULL`), la petición vuelve limpia y sin filas. Quien llame
  // tiene que poder distinguir «guardado» de «no había dónde guardarlo».
  const updateLiveSession = useCallback<DataState["updateLiveSession"]>(async (id, patch) => {
    const row = toSnakeRow(patch as Record<string, unknown>);
    const escribe = () => supabase.from("sessions").update(row).eq("id", id).eq("is_live", true).select("id");
    let { data, error } = await escribe();
    // El mismo reintento con token fresco que ya tenían los insert (D-081). Sin él, el
    // rollover normal del JWT —cada hora— habría encendido el aviso de sesión caducada.
    if (error && isRlsError(error)) {
      await forceRefresh();
      ({ data, error } = await escribe());
    }
    if (error) throw error;
