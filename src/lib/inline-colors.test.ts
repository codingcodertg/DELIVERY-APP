...
describe("Entregas: colores a pelo por fichero, techo de la decisión", () => {
  const TECHO_ENTREGAS: Record<string, number> = {
    ...
    // 2 desde D-268: los mismos dos negros que la Cuenta de Entregas tenía, el fondo detrás del video
  ...
  it("el total es 75, y de esos 61 son el blanco sobre color", () => {
    ...
    // 76 y 62 hasta D-NEXT: −1 blanco, el de la lista de facturas ajenas de «Misma factura», que ya no existe.
    let total = 0;
    let blancos = 0;
    for (const ruta of ficherosEntregas) {
      const h = coloresAPelo(leer(ruta));
