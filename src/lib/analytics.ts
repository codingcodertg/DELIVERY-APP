...
export function computeKpis(deliveries: Delivery[]): Kpis {
  ...
  return {
    ...
    // Los tres, de su función (D-363). El de pallets llevaba `Math.round` **a entero**, que es el
...
export function driverKpis(deliveries: Delivery[], capacityOf: (driver: string) => number, cost?: CostModel): DriverKpi[] {
  ...
  return [...map.entries()]
    .map(([driver, a]) => {
      ...
      // Los tres totales de este chofer, cada uno de su función y una sola vez (D-363).
