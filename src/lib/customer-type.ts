...
/** Lo que dice la cuenta guardada: Builder si está marcada; Mostrador si no lo está, o si no hay cuenta. */
export function tipoDeClientePorDefecto(cuenta: string | null | undefined, cuentas: readonly AccountRecord[] | null | undefined): CustomerType {
  const nombre = (cuenta ?? "").trim().toLowerCase();
  const guardada = nombre ? (cuentas ?? []).find((a) => (a.name ?? "").trim().toLowerCase() === nombre) : undefined;
  return guardada?.customer_type === "builder" ? "builder" : "counter_sale";
}

type OrdenConTipo = Pick<Partial<Delivery>, "order_type" | "account" | "customer_type">;

/** El tipo de cliente de una orden: el que alguien eligió, y si no, el de su cuenta. `null` si la orden
 *  no va a un cliente. */
export function tipoDeClienteDeLaOrden(d: OrdenConTipo, reglas: OrderTypeRules | undefined, cuentas: readonly AccountRecord[] | null | undefined): CustomerType | null {
  if (!esTipoDeCliente(d.order_type, reglas)) return null;
  return esValido(d.customer_type) ? d.customer_type : tipoDeClientePorDefecto(d.account, cuentas);
}
...
export function parcheDeTipoDeCliente(
  d: OrdenConTipo, reglas: OrderTypeRules | undefined, cuentas: readonly AccountRecord[] | null | undefined, ordenesCargadas: readonly object[],
): { customer_type?: CustomerType | null } {
  if (!laBaseTieneCustomerType(ordenesCargadas)) return {};
  return { customer_type: tipoDeClienteDeLaOrden(d, reglas, cuentas) };
}
